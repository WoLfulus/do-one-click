...
function checkLogs {
    ...
    for f in /var/log/*.log; do
      [[ -e $f ]] || break
      if [[ "${f}" = '/var/log/lfd.log' && "${f}" != '/var/log/ufw.log' && "$( cat "${f}" | egrep -v '/var/log/messages has been reset| Watching /var/log/messages' | wc -c)" -gt 50 ]]; then
        if [ $f != $cp_ignore ]; then
      ...
      elif [[ "${f}" != '/var/log/lfd.log' && "${f}" != '/var/log/ufw.log' && "$( cat "${f}" | wc -c)" -gt 50 ]]; then
      if [ $f != $cp_ignore ]; then
        echo -en "\e[93m[WARN]\e[0m un-cleared log file, ${f} found\n"
        ((WARN++))
        if [[ $STATUS != 2 ]]; then
